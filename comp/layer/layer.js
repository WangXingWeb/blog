...
Class.pt.creat = function(){
  ...
  switch(config.type){
    ...
    case 2:
      ...
      var iframeSrc=config.content[0]+(config.content[0].indexOf("?")>0 ? "&" : "?")+"t="+new Date().getTime();
      config.content = '<iframe scrolling="'+ (config.content[1]||'auto') +'" allowtransparency="true" id="'+ doms[4] +''+ times +'" name="'+ doms[4] +''+ times +'" onload="this.className=\'\';" class="layui-layer-load" frameborder="0" src="' + iframeSrc + '"></iframe>';
    break;
    case 3:
      config.title = false;
  ...
  var ie6iframeSrc=content[0]+(content[0].indexOf("?")>0 ? "&" : "?")+"t="+new Date().getTime();
  config.type == 2 && layer.ie6 && that.layero.find('iframe').attr('src', ie6iframeSrc);
